...
def check_term_start(dict_list):
    ...
    for dict_item in dict_list:
        if not dict_item["term_start"]:
            continue
        else:
            if pre_notnull_dict_item:
                if dict_item["term_start"] != pre_notnull_dict_item["term_start"]:
                    output.append((dict_item["ad"], dict_item["name"], dict_item["term_start"], dict_item["urls"]["ly"]))
...
empty_term_start = [(legislator["ad"], legislator["name"], legislator["urls"]["ly"]) for legislator in objs if not legislator["term_start"]]
